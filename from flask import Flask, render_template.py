...
@app.route('/cgpa')
def cgpa():
    """Calculate overall CGPA and store results."""
    if 'username' not in session:
        flash("Please log in to view results.", "error")
        return redirect(url_for('login'))
    ...
    with sqlite3.connect('users.db') as conn:
        c = conn.cursor()
        c.execute("INSERT INTO results (username, sgpa, cgpa, percentage) VALUES (?, ?, ?, ?)",
                  (session['username'], str(session['sgpa']), overall_cgpa, percentage))
        conn.commit()
...
def view_results():
    ...
    with sqlite3.connect('users.db') as conn:
        c = conn.cursor()
        c.execute("SELECT sgpa, cgpa, percentage FROM results WHERE username = ? ORDER BY id DESC LIMIT 1",
                  (session['username'],))
        result = c.fetchone()

    if result:
        sgpa, cgpa, percentage = result
        return render_template('view_results.html', sgpa=sgpa, cgpa=cgpa, percentage=percentage)
    else:
